...
class JoystickState
{
    ...
    private:
        ...
        float gamepadAxisCmd [MAX_JOYSTICK_AXES];
        int gamepadAxisCount;
        float gamepadDeadband[MAX_JOYSTICK_BUTTONS];
        float gamepadShaping[MAX_JOYSTICK_BUTTONS];
        int gamepadPOVCount;
        int flightcontrollerAxisCount;
        float flightcontrollerDeadband[MAX_JOYSTICK_BUTTONS];
        float flightcontrollerAxisCmd [MAX_JOYSTICK_AXES];
        float flightcontrollerShaping[MAX_JOYSTICK_BUTTONS];
    ...
    public:
        ...
        float GetGamepadAxis(int btnNumber);
        float GetFlightControllerAxis(int btnNumber);
        ...
        void SetGamepadAxisParameter(int axisNum, float deadband, float shaping);
        void SetFlightControllerAxisParameter(int axisNum, float deadband, float shaping);
    private:
        ButtonState BtnStateMachine(bool curPressed, ButtonState curState);
        float AxisShaping(float rawReading, float deadband, float shapingSlope);
